...
\begin{definition}[Plane in 3D Space]
    ...
    \[ax + by + cz + d = 0\]
    where the vector $\left<a, b, c\right>$ is the normal vector to the plane and $d$ is a displacement constant.
\end{definition}

\begin{remark}
    The above is equivalent to the form
    \[a(x - x_{0}) + b(y - y_{0}) + c(z - z_{0}) = 0\]
    that includes some point on the plane $(x_{0}, y_{0}, z_{0})$.
\end{remark}
